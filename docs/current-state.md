...
- **Sync** (`lib/idxSync.ts`, `runIdxSync`): Query 2 = all Active/Pending/Closed feed-wide (run FIRST — the big one); Query 1 = your offices (per-field `*OfficeMlsId in (...)` — split to stay under IIS's ~2KB URL limit); both incremental on `ModificationTimestamp` since the DB's max cursor, upserted **page-by-page** (flat memory, progress persists if cut short). Defensive field mapping (`mapRealcompListing`): city from `OriginalPostalCity`, county humanized, `WaterfrontFeatures` enum→CSV, Media→photos. No stale deactivation.
