...
- **Outbound** (`lib/agentSms.ts sendAgentSms`), always from the agent's home-office `offices.telnyx_number` (fallback `TELNYX_DEFAULT_FROM`): a reply-based **offer teaser** (no PII) on a new offer, the **full client-info text** on accept/manual-assignment (`lib/clientInfoSms.ts sendClientInfoSms`, shared by the web accept-flow and the inbound `YES` command), and an **update-due reminder** at the 48h first-update-overdue point (the `followup-check` cron). Every send/skip/failure is logged to `sms_messages`.
...
- **Owner setup (not code):** provision one Telnyx number per office (4 at launch) + complete 10DLC/toll-free carrier registration, set `TELNYX_API_KEY`/`TELNYX_PUBLIC_KEY`(/`TELNYX_MESSAGING_PROFILE_ID`/`TELNYX_DEFAULT_FROM`), populate `offices.telnyx_number`, point the Telnyx portal's inbound webhook at `/api/webhooks/telnyx`. Full walkthrough in `SETUP.md` §7. Live send/receive could not be exercised in this code-only session — no Telnyx credentials in the sandbox — so this is the owner's first-connection step, same as the IDX feed and Google Places before it.
...
- Verified at build time (Telnyx agent-texting session): typecheck clean, build compiles (38 static/dynamic routes incl. `/api/webhooks/telnyx`), 127 tests pass across 14 files.
