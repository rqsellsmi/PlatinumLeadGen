...
### 4.9 Buyer side — home search + buyer-lead pipeline (`feature/buyer-search`)
The buyer half of the platform (migrations **0031–0034**; design/plan in
`docs/superpowers/specs|plans/2026-07-25-buyer-search*`). Built on the same IDX
mirror + routing/scoring engine as the seller side, activated by `leads.intent`.
- **Buyer-first homepage** (`app/page.tsx`, `components/home/HomeSearchHero` +
  `HomeActiveListings` + `BuyerCityTiles`): a tabbed search hero (location search
  default + a "What's my home worth?" tab that launches the unchanged valuation
  flow), 9 most-recent active listings (service-area), and 12 city tiles
  (`lib/buyerHome.ts getBuyerCityTiles` — near-an-office ∧ most-active, minus
  `notification_settings.buyer_excluded_cities`, an **admin-editable, tiles-only**
  exclusion list seeded Flint/Pontiac/Detroit; excluded cities still resolve in
  search). The seller homepage moved intact to **`/sell/home-value`**
  (`components/home/SellerHomepage.tsx`).
- **Home search** (`/homes`, `lib/idxSearch.ts searchListings` + client-safe pure
  helpers in `lib/listingSearch.ts`): compliance-gated (Active + ActiveUnderContract
  only, `canDisplay`/`notLease`/`gateAddress`), filters for price/beds/baths/sqft/
  year/type/lot/garage/HOA/DOM/waterfront/pool/new-construction (never the 6 NULL
  Realcomp columns). `SearchFilterPanel` (URL-driven) + an interactive **Maps JS +
  Drawing** map (`SearchMap`: result pins, draw-an-area polygon, use-my-location,
  city autocomplete; address-hidden pins coarsened via `coarsenPin`). `IdxListingCard`
  now shows the **real** status (AUC renders its `mlsStatus`, never "For Sale").
  Migration **0031** adds the lat/lng/beds/baths/sqft/year/DOM search indexes.
- **Listing detail** (`/listing/[listingKey]`): key-feature icon chips from the
  listing's own populated data (`lib/listingFeatures.ts buildKeyFeatures` +
  `KeyFeatureChips`), a photo carousel in the hero slot + click-to-enlarge lightbox
  (`ListingHeroCarousel` + `PhotoLightbox`), a free-Embed location map below the
  remarks (`ListingLocationMap`, coarse when the address is hidden), and a **sticky
  "Schedule a showing" / "Contact an agent"** bar (`ListingContact`) for for-sale
  listings.
- **Buyer lead intake** (`POST /api/buyer/inquiry`, `lib/buyerInquiry.ts`;
  migration **0033** adds `leads.interested_listing_key`, `appointment_requests.
  listing_key`, `lead_type='buyer_inquiry'`): a buyer becomes a lead
  (`intent='buyer'`) carrying the **listing's** coordinates, deduped by email (one
  active buyer lead; extra interest attached as a `buyer_inquiry` event), routed via
  the existing `autoOfferLead` (proximity to the listing, out-of-area → admin).
  Showing requests also write `appointment_requests` (`source='showing'`).
- **Buyer-track scoring** (`lib/trackConfig.ts`; migration **0034** adds the
  `buyer_*` score reasons): one engine, two tunable configs selected by
  `leads.intent`. `SELLER_TRACK` = the unchanged v4; `BUYER_TRACK` reuses the seller
  `lead_status` values, **drops `appointment_set`**, uses buyer-specific Lost reasons
  (free-text — `leads.lost_reason` is a varchar), allows **`closed → nurturing`**
  (repeat client), and pays `buyer_attempted/connected/signed/closing/
  fast_engagement` (point values in `SCORE_DELTAS`, initialized as a seller mirror,
  tunable independently). `recordStatusUpdate` branches on intent; the agent status
  form + lead detail are intent-aware and link to the inquired listing. Shared four
  score tracks + one rotation queue (everyone gets both lead types).
- **Maps loader**: `lib/googleMaps.ts loadGoogleMaps()` — a single-load singleton
  (superset `places,drawing,geometry`) that `HeroValuation`, `ValuationForm`, and
  `SearchMap` all await, so the Maps JS API loads exactly once per page.
