...
- Agent = magic-link (64-hex, 30-day) or email+password, plus a signed HMAC session cookie (edge-verified in middleware). **Agents set/reset their own password** at the public **`/agent/set-password`** page (migration 0029): gated by the shared **agent setup code** (Admin → Settings) + the email being on the roster (active status not required). One URL for the whole team; the login page's "First time / forgot password" links here. Admin can still set a password directly on the agent editor. `/agent/set-password` is exempted from the agent-session middleware guard.
