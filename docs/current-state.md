...
- Verified at build time (queue head start + portal score session): typecheck clean, build compiles, 132 tests pass across 14 files (up from 127 — the Telnyx agent-texting session's baseline — with the starting-credit case added to `tests/v16.test.ts`).
